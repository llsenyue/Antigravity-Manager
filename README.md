...
* **版本演进 (Changelog)**:
  * **🚀 Fork 增强功能 (by @llsenyue)**:
        > 以下功能为本 Fork 相对上游的领先更新，尚未合并到上游仓库。
    * **智能预热机制 (Smart Warmup)**:
      * **配额感知预热**: 只预热配额 100% 的模型，<100% 的自动跳过（已在冷却期），避免无效消耗。
      * **图片模型优化**: 使用 `responseModalities: ["TEXT"]` + `maxOutputTokens: 1` 最小化图片模型预热消耗。
      * **单账号预热**: 在账号操作列新增预热按钮，支持对单个账号执行预热操作。
      * **预热后自动刷新**: 预热完成 5 秒后自动刷新所有账号配额，实时反映变化。
      * **智能消息反馈**: 预热后显示后端实际返回消息（如"所有模型已在冷却周期中，无需预热"）。
    * **API 监控看板增强**:
      * **修复日志丢失**: 修复 SQL 查询缺失 `mapped_model` 列导致的监控日志丢失问题。
      * **筛选逻辑修复**: 修复 Gemini/错误筛选条件判断逻辑。
      * **账号筛选器**: 新增按账号筛选请求日志功能。
    * **响应头增强**:
      * **X-Account-Email**: 在 OpenAI/Claude/Gemini 响应头中显示使用的账号邮箱。
      * **X-Mapped-Model**: 显示请求实际映射到的模型 ID，便于调试和监控。
    * **冷却指示器**:
      * 在账号列表和网格视图中使用 ❄️ emoji 标识配额 <100% 的模型冷却状态。
    * **定时预热调度器修复**:
      * 修复支持 `default` 键和 `enabled` 字段的配置解析问题。
  * **v3.3.15 (2026-01-04)**:
