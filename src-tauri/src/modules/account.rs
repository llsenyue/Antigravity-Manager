...
pub async fn fetch_quota_with_retry(account: &mut Account) -> crate::error::AppResult<QuotaData> {
    ...
    // 2. 尝试查询（优化：传入缓存的 project_id 以减少 loadCodeAssist 调用）
    // [FIX] 如果缺少 subscription_tier，强制不使用缓存以触发 loadCodeAssist 获取 tier
    let has_tier = account
        .quota
        .as_ref()
        .and_then(|q| q.subscription_tier.as_ref())
        .is_some();
    let cached_project_id = if has_tier {
        account.token.project_id.as_deref()
    } else {
        modules::logger::log_info(&format!("账号 {} 缺少订阅层级，强制刷新...", account.email));
        None
    };

    let mut result: crate::error::AppResult<(QuotaData, Option<String>)> =
    ...
    if let Ok((ref mut q, ref project_id)) = result {
        // [FIX] 如果使用了缓存导致没有返回 tier，保留旧的 tier
        if q.subscription_tier.is_none() {
            if let Some(old_quota) = &account.quota {
                if let Some(tier) = &old_quota.subscription_tier {
                    // tracing::debug!("保留现有订阅层级: {}", tier);
                    q.subscription_tier = Some(tier.clone());
                }
            }
        }
    ...
    if let Err(AppError::Network(ref e)) = result {
        if let Some(status) = e.status() {
            if status == StatusCode::UNAUTHORIZED {
                ...
                // 重试查询（也使用缓存的 project_id - 除非缺少 tier）
                // [FIX] 强制刷新 tier
                let has_tier = account
                    .quota
                    .as_ref()
                    .and_then(|q| q.subscription_tier.as_ref())
                    .is_some();
                let retry_cached_project_id = if has_tier {
                    account.token.project_id.as_deref()
                } else {
                    None
                };

                let mut retry_result: crate::error::AppResult<(QuotaData, Option<String>)> =
                    modules::fetch_quota_with_cache(
                ...
                if let Ok((ref mut q, ref project_id)) = retry_result {
                    // [FIX] Retry success, preserve tier if needed
                    if q.subscription_tier.is_none() {
                        if let Some(old_quota) = &account.quota {
                            if let Some(tier) = &old_quota.subscription_tier {
                                q.subscription_tier = Some(tier.clone());
